...
{ 
    ...
    class ChatServer
    {
        ...
        public static void AddUser(TcpClient tcpUser, string strUsername)
        {
            ...
            userNames[arrayIndex++] = strUsername;
        
            // Tell of the new connection to all other users and to the server form
            SendAdminMessage("newClient " + htConnections[tcpUser]);
        ...
        public static void SendAdminMessage(string Message)
        {
            ...
            e = new StatusChangedEventArgs("Adminstrator : " + Message);
            OnStatusChanged(e);
        ...
        public static void SendMessage(string From, string Message)
        {
            ...
            ChatServer.htUsers.Values.CopyTo(tcpClients, 0);
            // Loop through the list of TCP clients
            string toSend = "still unknown";
            string sendableMessage = "this message will be sent"; 
            ...
            if(Message.Equals("remove"))
            {
                ...
                for (int p = 0; p < tcpClients.Length; p++)
                {
                     // Try sending a message to each
                        try
                        {
                            // If the message is blank or the connection is null, break out
                            if (Message.Trim() == "" || tcpClients[p] == null)
                            {
                                continue;
                            }
                            // Send the message to the current user in the loop

                            swSenderSender = new StreamWriter(tcpClients[p].GetStream());
                            swSenderSender.WriteLine("NewClientt" + userNames[p]);
                            swSenderSender.Flush();
                            swSenderSender = null;
                ...
                for (int j = 0; j < tcpClients.Length; j++)
                {    
                    for (int i = 0; i < tcpClients.Length; i++)
                    {
                        if (htUsers.Contains(userNames[i]))
                        {
                            // Try sending a message to each
                            try
                            {
                                ...
                                swSenderSender = new StreamWriter(tcpClients[j].GetStream());
                                swSenderSender.WriteLine("newClientt" + userNames[i]);
                                swSenderSender.Flush();
                                swSenderSender = null;
            ...
            else if(Message.Equals("send to all") && tcpClients.Length>1)
            {
            ...
            { if(Message[i]=='|')
            {
                toSend = Message.Substring(i + 1, Message.Length - i - 1);
                sendableMessage = Message.Substring(0, i);
            }
            }
    ...
    class Connection
    {
        ...
        private void AcceptClient()
        {
            ...
            if (currUser != "")
            {
                ...
                {
                    ChatServer.SendMessage("function", currUser+"");
                    // 1 means connected successfully
                    swSender.WriteLine("1"+currUser);
                    ...
                    ChatServer.SendMessage(currUser, "send to all");
                }
            }
            else
            {
